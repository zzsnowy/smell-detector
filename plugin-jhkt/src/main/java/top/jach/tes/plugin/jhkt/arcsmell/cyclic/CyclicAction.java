...
public class CyclicAction implements Action {
    ...
    //寻找环回路信息
    private static void findCycle(int v,List<Integer> trace,Set<Integer> flags, List<String> result,List<String> nodes,int[][] matrix)
    {
        int j;
        //添加闭环信息
        ...
        for(int i=0;i<nodes.size();i++){
            if(matrix[v][i]==1 && !flags.contains(i)) {
                findCycle(i,trace,flags, result,nodes,matrix);
            }
        }
