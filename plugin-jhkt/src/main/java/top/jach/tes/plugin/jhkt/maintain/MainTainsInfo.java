...
//根据时间段获取可维护性指标
    ...
    public static MainTainsInfo newCreateInfo(Long reposId,
        ...
        for (Microservice microservice:
            ...
            mainTain.setElementName(microservice.getElementName());
            GitCommitsForMicroserviceInfo gitCommitsForMicroserviceInfo = gitCommitsForMicroserviceInfoMap.get(microservice.getElementName());
            //存在没有提交信息的微服务，若无提交信息，则事先做一次初始化为0，防止NullPointer出现
            if(gitCommitsForMicroserviceInfo==null){
                mainTain.setBugCount(0l);
                mainTain.setCommitAddLineCount(0l);
                mainTain.setCommitCount(0l);
                mainTain.setCommitDeleteLineCount(0l);
                mainTain.setCommitOverlapRatio(0d);
                mainTain.setCommitFilesetOverlapRatio(0d);
                mainTain.setPairwiseCommitterOverlap(0d);
            }else{
                // 每个微服务下的bug数
                long dtsCount = 0;
